...
try {
    ...
    $stmt = $conn->prepare("SELECT id FROM club_positions WHERE name = ? AND id != 0");
    $stmt->execute([$name]);
    if ($stmt->fetch()) {
    ...
    // Insert
    $stmt = $conn->prepare("
        INSERT INTO club_positions (name, level, description, is_active)
        VALUES (?, ?, ?, ?)
    ");
    
    $stmt->execute([$name, $level, $description, $is_active]);
